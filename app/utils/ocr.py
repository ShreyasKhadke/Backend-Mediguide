...
# Explicitly set Tesseract path for Windows - Moved to function level if needed
# if os.name == "nt": ...
...
class OCRService:
    ...
    async def _tesseract_ocr(self, image_data: bytes) -> str:
        ...
        return await loop.run_in_executor(
            None, self._run_tesseract_sync, image_data
        )
    ...
    def _run_tesseract_sync(self, image_data: bytes) -> str:
        ...
        logger.info(
            f"OCR: Starting Tesseract execution on {len(image_data)} bytes"
        )
        # Open image
        ...
        text = pytesseract.image_to_string(
            image, lang="eng", config=custom_config
        )
        ...
        if not text.strip():
            raise Exception(
                "No text could be extracted from the image. Please ensure the image is clear and readable."
            )
    ...
    def parse_structured_data(self, text: str) -> dict:
        ...
        for i, line in enumerate(lines):
            ...
            has_medical_keyword = any(
                keyword in line_lower for keyword in medical_keywords
            )
        ...
        if not parameters and text.strip():
            ...
            for line in lines:
                ...
                if re.search(r"\d+", line) and len(line.split()) >= 2:
                    ...
                    if any(
                        re.match(r"^\d+", p.replace(",", "")) for p in parts
                    ):
                        # Try to extract
    ...
    def _detect_report_type(self, text: str) -> str:
        ...
        if (
            "cbc" in text_lower
            or "complete blood count" in text_lower
            or "haematology" in text_lower
        ):
    ...
    def _extract_lab_name(self, text: str) -> str:
        ...
        for line in lines:
            if any(
                ...
                for keyword in ["lab", "diagnostics", "laboratory", "clinic"]
            ):
                return line.strip()
        return "Unknown Lab"
